...
from func.utils import check_memory, set_seed, PCA_reduction
...
RESULTS_ROOT = "./results/mTRF-PCA_iterate-model_sub"
    ...
    if args.char:
        ...
        for sub in ["sub-01","sub-02","sub-03","sub-04","sub-05",
            ...
            for model_type in model_paths.keys():
                ...
                embeds = PCA_reduction(np.concatenate(embeds, axis=0), n_components=VAR_THRES, if_scale=True).astype(DTYPE) # N, 10, D
    ...
    if args.word:
        ...
        for sub in ["sub-01","sub-02","sub-03","sub-04","sub-05",
            ...
            for model_type in model_paths.keys():
                ...
                for run in range(1, 51):
                    embeds.append(np.load(os.path.join(embeds_dir, f"whisper_{FEATURE}_context-{FIXED_CONTEXT_LEN}_story_{run}.npy")))
                embeds = PCA_reduction(np.concatenate(embeds, axis=0), n_components=VAR_THRES, if_scale=True).astype(DTYPE)
